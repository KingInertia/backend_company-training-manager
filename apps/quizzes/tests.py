...
class AnaliticTestCase(APITestCase):
    ...
    def test_scores_for_all_quiz_over_time(self):
        self.client.force_authenticate(user=self.user)
        start_date = '2024-12-10'
        end_date = '2024-12-19'
        filter_type = 'quiz'
        response = self.client.get(
            f'/api/v1/quizzes/dynamic-scores/?start_date={start_date}&end_date={end_date}&type={filter_type}',
        )
        quiz1_avg_score = (self.user_quiz_result1.correct_answers / self.user_quiz_result1.total_questions + 
                           self.user_quiz_result3.correct_answers / self.user_quiz_result3.total_questions +
                           self.user_quiz_result5.correct_answers / self.user_quiz_result5.total_questions) / 3
        quiz2_avg_score = (self.user_quiz_result2.correct_answers / self.user_quiz_result2.total_questions +
                           self.user_quiz_result4.correct_answers / self.user_quiz_result4.total_questions +
                           self.user_quiz_result6.correct_answers / self.user_quiz_result6.total_questions) / 3
        current_date = datetime.now().strftime('%Y-%m-%d') + "T00:00:00Z"
        expected = [
            {
                'id': self.quiz1.id,
                'dynamic_time': [
                    {'day': current_date, 'average_score': round(quiz1_avg_score, 2) * 100}
                ]
            },
            {
                'id': self.quiz2.id,
                'dynamic_time': [
                    {'day': current_date, 'average_score': round(quiz2_avg_score, 2) * 100}
                ]
            }
        ]
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_scores_for_user_over_time(self):
        self.client.force_authenticate(user=self.user)
        start_date = '2024-12-10'
        end_date = '2024-12-19'
        ...
        response = self.client.get(
            f'/api/v1/quizzes/dynamic-scores/?start_date={start_date}&end_date={end_date}&user_id={user_id}',
        )
        
        quiz1_avg_score = (
            self.user_quiz_result1.correct_answers / self.user_quiz_result1.total_questions +
            self.user_quiz_result5.correct_answers / self.user_quiz_result5.total_questions
        ) / 2
        
        quiz2_avg_score = (
            self.user_quiz_result2.correct_answers / self.user_quiz_result2.total_questions
        )
