...
class CompanyInvitationViewSet(viewsets.ModelViewSet):
    ...
    def decline_invitation(self, request, *args, **kwargs):
        ...
        invitation.status = RequestState.DECLINED
        invitation.save()

        return Response(CompanyInvitationSerializer(invitation).data)

    @action(detail=True, methods=['patch'], url_path='cancelled', permission_classes=[IsOwnerOfCompany])
    def cancelled_invitation(self, request, *args, **kwargs):
        ...
        invitation.status = RequestState.CANCELLED
        invitation.save()
...
class CompanyRequestViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['patch'], url_path='accept', permission_classes=[IsOwnerOfCompany])
    def accept_request(self, request, *args, **kwargs):
        request_obj = self.get_object()

        if request_obj.status != RequestState.PENDING:
            return Response({"detail": "Request already processed."}, status=status.HTTP_400_BAD_REQUEST)

        request_obj.status = RequestState.ACCEPTED
        request_obj.save()
    ...
    @action(detail=True, methods=['patch'], url_path='decline', permission_classes=[IsOwnerOfCompany])
    def decline_request(self, request, *args, **kwargs):
        ...
        request_obj.status = RequestState.DECLINED
        request_obj.save()
    ...
    def cancelled_request(self, request, *args, **kwargs):
        ...
        request_obj.status = RequestState.CANCELLED
        request_obj.save()
